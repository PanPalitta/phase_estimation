...
int main(int argc, char **argv){
	...
	if(N_cut<N_begin){cout<<"please select new N_cut>"<<N_begin<<":";cin >> N_cut;}
	else{}
	...
	for(p=0;p<nb_proc;p++){can_per_proc[p]=0;}//make sure the array started with zeroes.
	for(p=0;p<pop_size;p++){can_per_proc[p%nb_proc]+=1;}


	if(my_rank==0){output_header();}else{}//write header of result files
	...
	if(numvar<N_cut){opt->Init_population(can_per_proc[my_rank]);}
	//create candidates according to how many should be on that particular processor.
		...
		if(my_rank==0){
			for(p=1;p<nb_proc;p++){MPI_Send(&solution[0],numvar,MPI_TYPE,p,tag,MPI_COMM_WORLD);}
			}
		else{MPI_Recv(&solution[0],numvar,MPI_TYPE,0,tag,MPI_COMM_WORLD,&status);}
		...
		if(numvar<T_cut_off){opt->set_success(iter_begin,0);}
		else if(numvar>=data_end){opt->set_success(1000,1);}//stop after perform 1000 iteration or exceeding the goal.
		else {opt->set_success(iter,0);}
        ...
        if(my_rank==0){output_result(numvar,final_fit,solution,start_time);}else{}
        ...
        if(numvar==data_end-1){opt->linear_fit(data_size,x,y,&slope,&intercept);}else{}
