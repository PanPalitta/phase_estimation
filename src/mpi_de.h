...
class DE : public OptAlg<typeT> {
	public:
		DE(){};
		DE(Problem<typeT> *problem_ptr):F(0.1),Cr(0.6){this->prob=problem_ptr;this->num=this->prob->num;}
		~DE(){};
...
void DE<typeT>::combination(int my_rank, int total_pop, int nb_proc){
	...
	if(typeid(input[0])==typeid(double)){MPI_TYPE=MPI_DOUBLE;}
	else if(typeid(input[0])==typeid(dcmplx)){MPI_TYPE=MPI_DOUBLE_COMPLEX;}
	else{}
	...
	for(p=0;p<total_pop;p++){
		if(p%nb_proc!=0){//if candidate is not in root, send the solution to root.
			if(my_rank==p%nb_proc){
				...
				for(i=0;i<this->num;i++){all_soln[p][i]=input[i];}
				}
			}
	...
	for(p=0;p<total_pop;p++){
		...
		if(my_rank==0){
            ...
            if(total_pop<=fam_size+1){
                for(f=0;f<fam_size;f++){fam[f]=rand()%total_pop;}
            }
            else{
                do fam[0]=rand()%total_pop; while(fam[0]==p);
                do fam[1]=rand()%total_pop; while(fam[1]==p||fam[1]==fam[0]);
                do fam[2]=rand()%total_pop; while(fam[2]==p||fam[2]==fam[0]||fam[2]==fam[1]);
            }
			//create donor
			...
			//if(fam_size==3){//check that we have the combination rule
				for(i=0;i<this->num;i++){
					...
					if(coin<=Cr){can[i]=input[i];}
					else{can[i]=all_soln[p][i];}
					}
				//}
		...
		for(p=0;p<this->pop_size;p++){this->Cont_fitness(p);}//compute the fitness
