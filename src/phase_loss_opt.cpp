...
double Phase::fitness(double *soln){
	...
	for(k=0;k<K;k++){
		...
		PHI=0;
		/*copy input state*/
		for (m=0;m<=num;m++){state[m]=input_state[m];}
		/*measurement*/
		d=0;
		for (m=0;m<num;m++){
			...
			if(coin<=loss){
				...
				if(dect_result==0){PHI=PHI-soln[d];}
				else{PHI=PHI+soln[d];}
				PHI=mod_2PI(PHI);
				d++;
				}
...
void Phase::one_over_fac(double *over_mat){//calculate one over factorial matrix
	int i;
		for(i=0;i<=num;i++){
		if(i==0){over_mat[i]=1;}
		else{over_mat[i]=over_mat[i-1]/i;}
		}//end i
	}
...
void Phase::sqrtfac(double *fac_mat){//calculate sqrt of factorial matrix
	...
	for(i=0;i<=num;i++){
		if(i==0){fac_mat[i]=1;}
		else{fac_mat[i]=fac_mat[i-1]*sqrt(i);}
		}//end i
	}
...
double Phase::cal_spart(int n, int k, int N){//calculating the Wigner d-matrix element
	...
	//find lower limit
	if(n-k>=0){s_min=0;}
	else if(n-k<0){s_min=k-n;}
	else {}
	//find upper limit
	if(k<=N-n){s_max=k;}
	else if(k>N-n){s_max=N-n;}
	else {}
...
//N is the number of photons currently available, not equal to 'num'
	...
	//measurement outcome is 0
		for(n=0;n<N;n++){state[n]=update0[n]/sqrt(prob[0]);}
		state[N]=0;
		result=0;
		}
	else{//measurement outcome is 1
		for(n=0;n<N;n++){state[n]=update1[n]/sqrt(prob[1]);}
		state[N]=0;
		result=1;
	}
		return result;
...
void Phase::state_loss(int N){
	...
	for(n=0;n<N;n++){update0[n]=state[n]*sqrt(N-n)+state[n+1]*sqrt(n+1);}
	for(n=0;n<N;n++){state[n]=update0[n]*factor;}
	state[N]=0;
	}
