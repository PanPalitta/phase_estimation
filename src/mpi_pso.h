...
class PSO : public OptAlg<typeT> {
	public:
		PSO(){};
		PSO(Problem<typeT> *problem_ptr):w(0.8),phi1(0.6),phi2(1.0),v_max(0.2){this->prob=problem_ptr;this->num=this->prob->num;}
		~PSO(){};
...
void PSO<typeT>::put_to_best(int my_rank, int total_pop, int nb_proc){
	...
	for(p=0;p<this->pop_size;p++){
		...
		for(i=0;i<this->num;i++){array[i]=double(rand())/RAND_MAX*v_max;}
...
void PSO<typeT>::find_global(int my_rank, int total_pop, int nb_proc){
	...
	if(my_rank==p%nb_proc){
		...
		if(prev_fit<=fit){
			ptr=p;
			if(fit<forw_fit){ptr=forw;}
			else{}//ptr=p
			}
		else if(prev_fit>fit){
			//ptr=prev
			if(prev_fit<forw_fit){ptr=forw;}
	...
	//updating global best
		if(ptr==p){//Global best already in precessor's memory. No need for MPI
			...
			if(my_rank==ptr%nb_proc){
				...
				this->pop[p/nb_proc].update_global(array);
				if(ptr==prev){this->pop[p/nb_proc].write_globalfit(prev_fit);}
				else if(ptr==forw){this->pop[p/nb_proc].write_globalfit(forw_fit);}
				else{cout<<"Error!! ptr not pointing correctly"<<endl;}
				}
			}
		else{
		    //cout<<"ptr:"<<ptr<<",prev="<<prev<<",forw="<<forw<<endl;
...
void PSO<typeT>::combination(int my_rank, int total_pop, int nb_proc){
	...
	for(p=0;p<this->pop_size;p++){
		...
		for(i=0;i<this->num;i++){
			...
			if(vel[i]>v_max){vel[i]=v_max;}
			else if(vel[i]<-v_max){vel[i]=-v_max;}
			else{/*keep vel[i]*/}
			}
