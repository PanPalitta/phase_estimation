...
public:
	...
	double read_contfit(){return cont_fit;}
	double read_bestfit(){return best_fit;}
	double read_globalfit(){return global_fit;}
	int read_bestt(){return best_times;}
	
	void write_contfit(double fit,int tt){cont_fit=fit/tt;times=tt;}
	void write_bestfit(double fit){best_fit=best_fit*best_times+fit;best_times+=1;best_fit=best_fit/best_times;}
	void write_globalfit(double fit){global_fit=fit;}
...
void Candidate<typeT>::update_cont(typeT *input){
	int i;
	for (i=0;i<num;i++){contender[i]=input[i];}
	}

template<typename typeT>
void Candidate<typeT>::update_vel(typeT *input){
	int i;
	for (i=0;i<num;i++){velocity[i]=input[i];}
	}
	
template<typename typeT>
void Candidate<typeT>::update_best(){
	int i;
	for (i=0;i<num;i++){can_best[i]=contender[i];}
	best_fit=cont_fit;
	best_times=times;
	}
...
void Candidate<typeT>::update_global(typeT *input){
	int i;
	for (i=0;i<num;i++){global_best[i]=input[i];}
	}

template<typename typeT>
void Candidate<typeT>::put_to_global(){
	int i;
	for (i=0;i<num;i++){global_best[i]=can_best[i];}
	global_fit=best_fit;
...
void Candidate<typeT>::read_cont(typeT *output){
	int i;
	for (i=0;i<num;i++){output[i]=contender[i];}
	}

template<typename typeT>	
void Candidate<typeT>::read_vel(typeT *output){
	int i;
	for (i=0;i<num;i++){output[i]=velocity[i];}
	}

template<typename typeT>
void Candidate<typeT>::read_best(typeT *output){
	int i;
	for (i=0;i<num;i++){output[i]=can_best[i];}
	}

template<typename typeT>
void Candidate<typeT>::read_global(typeT *output){
	int i;
	for (i=0;i<num;i++){output[i]=global_best[i];}
	}
